...
void usage(const char* progname) 
{
    ...
    printf("  -d  --delta <number>                                    Set delta to use\n");
    printf("  -c  --check                                             Set this to verify correctness against sequential Dijkstra's\n");
...
int main(int argc, char** argv)
{
    ...
    while ((opt = getopt_long(argc, argv, "a:v:d:c?", long_options, NULL)) != EOF) 
    {
        switch (opt) 
        {
        case 'a':
